...
func _ready() -> void:
	$WaitTimer.connect("timeout", self, "_on_wait_timeout")
	$RelocateArea.connect("body_entered", self, "_on_body_entered")
	origin = enemy.global_position
	relocate_spot = origin

func physics_update(delta) -> void:
	$RelocateArea.global_position = enemy.global_position
